...
Terminal::Terminal(SDL_Renderer *context, int width, int height) {
  font = new TerminalFont(context, "assets/font.png", 128, 128, 8, 8);
  ...
  buffer.resize(width * height,
                (TerminalGlyph){(int)L'Я', (SDL_Color){255, 0, 0, 255},
...
void Terminal::Display(SDL_Renderer *context) {
  ...
  SDL_Rect bsrc = {(int)std::get<0>(atlas_bglyph_pos), (int)std::get<1>(atlas_bglyph_pos),
  ...
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      ...
      SDL_FRect dest = {(float)(j * tileWidth), (float)(i * tileHeight), (float)tileWidth, (float)tileHeight};
